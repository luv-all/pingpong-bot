...
**상태**: Phase 0·Phase 1·WP9(커밋 `42b8333`)·WP2b(로컬 완료, 아래 §0.3 참고)
완료. Phase 2 잔여는 **WP4a·WP3 미착수**.
...
Phase 0·Phase 1·WP9·WP2b 완료(§0 참고). **다음 세션은 여기서 시작**:

1. WP2b(§0.3)가 밝힌 진짜 병목("세기 1.5배 부족, 랭킹·조준 문제 아님")을
   정식 WP로 승격할지 사용자 확인 — §0.3 "후속 과제" 4항 중 착수 순서
   결정 필요.
2. WP4a(elbow-up 단일 config A/B)는 원안대로 진행 가능하나, §0.3의 세기
   병목이 A/B 결과(달성 `v_r·n`)를 왜곡할 수 있어 **병목 해소 후 재실행이
   안전** — 병목 후속작업과 순서 조율 필요.
3. WP3(좌우 중앙 타겟팅)는 §0.3에서 "조준 자체는 정상"으로 확인됐으므로
   원인 규명 목적보다는 후속작업 3항(리턴 타겟을 더 가깝게 잡아 필요
   `|v_out|` 낮추기)과 결합해 검토하는 편이 나음.
