...
사용자 제안, 채택)·WP4a(로컬 완료, §0.7 — 채택) 완료. Phase 2 원안 5개
WP(WP1·WP2a·WP2b·WP2c·WP4a·WP4c) **전부 완료**. 남은 건 §0.3 "후속 과제"·
§0.6 "후속 과제"에 적힌 열린 방향들(정식 WP 번호 미부여, 다음 세션
착수 여부 확인 필요) 뿐.
...
### 0.7 WP4a — IK를 elbow-up 단일 시드로 제한 (2026-07-30, 채택)

계획 원안대로 두 가지를 실측했다.

**elbow-up 부호 규약 확정**(가정 없이 수치 검증): q2를 ±0.2 흔겨 라켓
z높이 변화를 쟀더니 **q2가 더 음수일수록 라켓이 올라간다** —
elbow-up = q2<0. 현재 기본 자세(`READY_JOINTS_4DOF`, q2=-0.2054)는 이미
elbow-up 쪽이고, 관절한계도 비대칭(음수 쪽이 훨씬 넓음)이라 이 arm의
주 동작 영역과 일치한다.

**단일 시드 vs 다중 시드(4개 반사) A/B**(WP11 적용 후 재측정): 150개
지오메트리에서 `base_hint`(시작=`default_joints`)가 **100% elbow-up으로
수렴**했고, 단일 시드가 실패하고 다중 시드만 성공한 사례는 **0건**.
다중 시드가 더 낮은 `r`을 찾은 경우(89건 중 50건)도 개선폭이 평균
Δr=0.0007(최대 0.003)로 사실상 0 — 2026-07-23에 기록된 "반사 시드로
조작성 최대 7배 차이" 문제를 WP11(자체운동)이 대부분 흡수했다는 뜻.

**변경**: `best_impact_candidate_for_outgoing`을 "elbow-up 단일 시드
우선 → 실패시에만(테이블관통 등으로 아무 후보도 못 만들 때만) 반사
시드 3개로 확장"으로 재구성(계획 원안의 절충안 그대로). 라이브 eval
그리드(고정 시드, `diag_wp2b_ab_metrics`)에서 `|v_out|/desired`가
0.673(WP11 이후)→**0.696**로 회귀 없이 추가 개선됐다 — 개별 후보 단위
`r`은 다중이 항상 단일의 상위집합이라 나빠질 수 없는데 집계가 더
좋아진 건, WP2b 랭킹이 근소한 순위 변동에 민감한 그리디 탐색이라는
점으로 설명은 되지만 완전히 검증된 인과는 아니다(정직하게 기록,
`docs/wp4a-ik-seed-manipulability.md` §4).

**계획 속도 이득은 이 벤치마크로는 못 잡았다**(1.01배, 사실상 차이
없음) — 합성 지오메트리 40.7%가 IK 실패 케이스라 폴백 경로(1차 실패
후 반사 3개 재시도)가 절감분을 상쇄했다. 실제 eval 그리드는 IK 성공률
100%라 이론상 더 큰 절감이 있겠지만 재측정하지 않고 정직하게
미확인으로 남겼다 — §4의 정확도 개선과 무회귀만으로 채택 근거는
충분하다고 판단.

**파일**: `src/robot/motion/impact_candidate.rs`(`best_impact_candidate_
for_outgoing` 2단계 재구성, 진단 테스트 3개), `src/robot/tests.rs`
(elbow-up 부호규약 진단 1개). 상세: `docs/wp4a-ik-seed-manipulability.md`.
...
Phase 0·Phase 1·WP9·WP2b·WP10·WP3·WP11·WP4a 전부 완료(§0 참고). 계획
원안의 Phase 2 항목은 남은 게 없다. 세기 격차 조사가 밝힌 사실 체인:
WP10(coarse 추종 무관) → WP3(요구크기 축소 무관, 방향-포즈 상호작용이
핵심) → WP11(같은 포즈 내 배분 최적화, 채택) → WP4a(포즈 탐색 자체는
이미 충분, 단일 시드로 단순화). **다음 세션은 여기서 시작**:

1. WP11 §6 "후속 과제"의 두 방향 중 착수 여부 확인 — (a) 레일까지
   포함한 전체 자체운동(2차원 널스페이스, 구현 복잡도↑), (b) WP4a와
   WP11이 이제 둘 다 자리 잡았으니 세기 격차가 얼마나 남았는지 재측정
   (`|v_out|/desired`가 0.696까지 왔지만 아직 1.0에는 못 미침 — 남은
   격차의 성격을 다시 데이터로 봐야 다음 레버를 고를 수 있다).
2. WP3 §5 "후속 과제"의 나머지 항목(랠리 리턴 타겟을 WP3가 아닌 다른
   방식으로 조정하는 안, 좌우 중앙 타겟팅 원안 질문)은 여전히 미착수.
3. `min_swing_secs` 추가 확대는 우선순위 하향 유지(WP10 근거,
   quintic이 이미 98% 무손실).
4. 실행 방식(순차 vs `team`)은 착수 시 별도 확인 — 이번 세션은 조직
   월간 API 지출 한도로 백그라운드 서브에이전트가 중단돼 WP3/WP11/WP4a를
   전부 직접(서브에이전트 없이) 진행했다. 한도가 풀렸는지 세션 시작 시
   확인 권장.
