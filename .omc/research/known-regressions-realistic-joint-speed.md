...
**최종 갱신: 2026-07-27 (4차 — bang-bang 디버그 경로 재진단: "수렴 불가"는
오진단, 진짜 원인은 스위칭 곡선 버그 + 관절/축 동기화 부재였음)**
...
| 4차 | §4-1의 "실기 한계상 수렴 불가"도 **오진단이었음을 실측 확인** — 위치는
완전 수렴하도록 고쳤으나 속도는 미해결로 남아 여전히 `#[ignore]` (상세: §4) |
...
1. `planner::bang_bang::plan_bang_bang_swing_converges_for_a_reachable_impact`
   — **2026-07-27 재조사로 이 항목의 원래 결론("실기 한계상 수렴 불가")이
   오진단이었음이 드러났다.** 실측(스위칭 곡선을 직접 추적): 관절별 독립
   bang-bang은 목표속도가 0이 아닌 관절이 "도달"한 뒤 곧 "지나쳐" 재보정을
   반복하는 진짜 수학적 결함(동기화 부재)이었지, 토크·속도 포화 자체가
   근본원인이 아니었다. 원인을 잡아가며 설계를 세 번 교체했다:
   (1) 관절별 독립 bang-bang → (2) 라켓(3D) 축별 독립 bang-bang(3축이
   자코비안 결합으로 서로 간섭해 거시적 진동) → (3) **ZEM/ZEV(Zero-Effort-Miss
   / Zero-Effort-Velocity) 유도**(위치·속도오차·목표속도를 하나의 벡터식으로
   묶어 진동 자체가 생길 수 없음, `Tg`를 실제 도달 시각에 고정해 동기화
   문제를 원천 해결) + 토크 가중 최소노름 자코비안 역산(토크 여유가 큰
   베이스 쪽 관절을 더 쓰도록).
   **결과 — 위치는 완전히 고쳐졌다, 속도는 미해결로 남았다.** `swing_bench`
   다중 시나리오 스윕(6개 중 5개 성공적으로 계획, 1개는 IK 도달 범위 밖):
   위치오차가 **모든 시나리오에서 0.00004~0.00007**(허용오차 1e-3의 5% 미만)
   까지 수렴 — 이전 버전들의 0.024~0.8 진동/발산과 질적으로 다르다. 하지만
   라켓 속도는 시나리오·목표속도(1.55~2.49 m/s)와 무관하게 **일관되게
   목표의 22~24%**에서 멈춘다(방향오차는 4.8~6.0°로 15° 허용오차 안 —
   방향은 문제없다). ZEM/ZEV는 시간이 넉넉하면 초반에 힘을 아끼는
   최소에너지 특성이 있어 `TIME_TO_GO_BIAS=0.5`로 보정했지만, 원래 짧은
   커밋창(≤0.35s)에서 이 격차를 완전히 없애지는 못했다. **속도 부족의
   재현성 있는(scenario-independent) 원인은 아직 특정되지 않음** — 다음
   조사 후보로 남긴다(§6).
   **quintic 게임플레이 경로(`plan_best_swing`)와 무관한 GUI 디버그 전용
   경로**라 사용자 영향 없음. 전체 기록: `.omc/progress.txt`,
   `.omc/plans/bang-bang-swing-lag-and-convergence-fix.md`.
...
3. **bang-bang 디버그 경로의 속도 부족 원인 조사** (§4-1, GUI 디버그 전용,
   사용자 영향 없음) — 위치는 완전히 수렴하는데 속도가 시나리오와 무관하게
   목표의 22~24%에서 멈추는 재현성 있는 패턴. 후보: (a) `TIME_TO_GO_BIAS`
   외에 ZEM/ZEV의 최소에너지 프로파일 자체를 더 근본적으로 바꾸는 방법(예:
   `Tg` 자체를 편향하는 대신 유도법을 바꿈), (b) 토크 가중 지수(현재
   `torque_limit⁴`)나 감쇠계수(`JACOBIAN_DAMPING=0.05`)의 추가 튜닝, (c) 이
   특정 팔의 토크 예산으로 0.5s(현재 `MAX_PLAN_TIME_SECS`) 안에 이 속도대가
   원리적으로 달성 불가능한지 검증(더 긴 예산으로 스윕해 한계 곡선을 그려볼
   것).
