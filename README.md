...
Planner::ball_alignment` 경로로 본 예측의 레일·팔 정렬 궤적을 계산한다. 후속 갱신은
레일을 고정하고 팔 자세만 다시 풀어
보정된 공 접촉점을 라켓 블레이드에 맞추고, 라켓 면은 네트 너머 상대편
탁구대 반코트의 무게중심을 향한다. 타격 0.25초 전 q2·q3 스윙을
시작해 예측 충돌 순간에 라켓 면 수직 방향 선속도를 만든다.
타격 후에는 준비 자세로 복귀하지 않고 마지막 타격 자세를 유지한다.
리니어 레일 `+4.0 cm`(기존 +2.5 cm에 +1.5 cm 추가)는 타격 목표가 아니라 AXL board↔domain 좌표계 보정으로 한 번만 적용한다.
...
예측 위치에 레일·전체 팔로 정렬하고, 타격 0.25초 전에 q2·q3를
돌려 임팩트 순간 라켓 면 수직 방향으로 밀어준 뒤 타격 자세를 유지한다.
...
전체 팔·레일 IK는 라켓 면을 15° 상향각으로 상대 코트 3/4 중심을 향하게 하며 예측 공 높이보다 1.5cm 높은 지점에 정렬한다. 타격 후에는 마지막 자세를 유지한다.
...
본 예측 첫 명령은 레일·전체 팔을 정렬하고, 같은 공의 후속 예측은
레일을 고정한 채 팔만 보정한다. 타격 0.25초 전 스윙을 시작해
충돌 순간 라켓 면 수직 방향으로 밀고, 타격 후에는 복귀 명령 없이 자세를 유지한다.
...
직전 보정 중 도착한 요청은 최신 하나만 보관해 이동 완료 직후 적용하며, 정렬·타격 중 생긴
