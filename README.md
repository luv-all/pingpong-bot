...
리니어 레일 x는 유지하고 관절만 높은 역 U자 아치 자세로 복귀한다.
...
로봇은 실기 안전 범위(0.0100~1.3395 m) 안에서 탁구대 실측 중앙 위치(0.675 m)와 사진 기준 높은 역 U자 기본자세로 시작한다. 시뮬 `Shoot`은 기존처럼 즉시 공을 발사한다.
...
예측 위치 뒤 최대 10cm에서 팔을 접어 정렬하고, 타격 0.20초 전에 백스윙 없이 j0~j3 직진 푸시를 시작한다. 타격 후에는 레일을 움직이지 않고 관절만 높은 역 U자 아치 자세로 복귀한다.
...
타격 0.20초 전에 접힌 정렬 자세에서 별도 백스윙 없이 j0~j3 직진 푸시를 시작한다. 10cm 전진 구간의 임팩트 목표 속도는 0.85m/s다. 임팩트 후 0.12초 팔로스루 뒤 리니어 레일은 현재 x에 두고 관절만 높은 역 U자 아치 자세로 복귀한다.
