...
상대편 끝선 중앙을 조준한 뒤, 공 도착 직전에 속도를 가진 짧은 고정 임팩트 푸시를
실행한다. GUI sim도 같은 정렬 계산과 임팩트 궤적 계획을 쓴다.
...
공 도착 0.12초 전에 2.5cm 고정 푸시를 시작해 0.65m/s 임팩트와 0.06초
팔로스루를 만들며, 적응형 전체 스윙·결선 랠리는 아직 지원하지 않는다. 이후
...
공 도착 직전에 짧은 고정 임팩트 푸시를 실행하고 팔로스루 후 중앙 준비 자세로 복귀한다.
