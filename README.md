...
로봇은 실기 안전 범위(0.0705~1.4100 m) 안에서 탁구대 실측 중앙 준비 위치(0.740 m)와 준비 관절 자세로 바로 이동한다.
...
시작 시 레일 0.740m와 기본 관절각의 중립 자세를 만든다. 공 검출 시
...
AXL 앱 안전 범위는 `0.0705~1.4100 m`이다. `reverse=true`라 도메인
좌표 증가와 보드 이동 방향이 반대이며, 준비 위치 `0.740 m`는
탁구대 실측 중앙 보정값이다.
