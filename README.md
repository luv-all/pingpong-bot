...
0.25초 전에 별도 백스윙 없이 j0~j3로 라켓을 바로 전진시킨다. 정렬된 라켓 면을
유지한 채 임팩트 후에도 0.12초 동안 같은 방향으로 팔로스루한 뒤,
리니어 레일 x는 유지하고 관절만 `main`의 준비 자세로 복귀한다.
...
로봇은 실기 안전 범위(0.0100~1.3395 m) 안에서 탁구대 실측 중앙 위치(0.675 m)와 `main`의 준비 관절 자세로 시작한다. 시뮬 `Shoot`은 기존처럼 즉시 공을 발사한다.
슈터 GUI로 발사하면 기본 경로는 월드 ground-truth 궤적에서 목표를 고른 뒤
실기와 같은 `Planner::ball_alignment`로 레일·팔을 함께 움직인다.
첫 검출 뒤 0.15초 동안 관측을 모은 다음, 검증된 `y=0.08~0.35m` 구간에서 타격점을 고른다.
예측 위치 뒤 최대 10cm에서 팔을 접어 정렬하고, 타격 0.25초 전에 백스윙 없이 j0~j3 직진 푸시를 시작한다. 타격 후에는 레일을 움직이지 않고 관절만 `main`의 준비 자세로 복귀한다.
...
타격 0.25초 전에 접힌 정렬 자세에서 별도 백스윙 없이 j0~j3 직진 푸시를 시작한다. 10cm 전진 구간의 임팩트 목표 속도는 0.70m/s다. 임팩트 후 0.12초 팔로스루 뒤 리니어 레일은 현재 x에 두고 관절만 `main`의 준비 자세로 복귀한다.
