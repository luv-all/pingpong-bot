...
탁구대 반코트의 무게중심을 향한다. 공 뒤 최대 10cm의 접힌 정렬 자세에서 예상 타격
0.40초 전에 별도 백스윙 없이 j0~j3로 라켓을 바로 전진시킨다. 정렬된 라켓 면을
유지한 채 임팩트 후에도 0.12초 동안 같은 방향으로 팔로스루한 뒤,
리니어 레일 x는 유지하고 관절만 안쪽으로 말린 높은 아치 자세로 복귀한다.
...
로봇은 실기 안전 범위(0.0100~1.3395 m) 안에서 탁구대 실측 중앙 위치(0.675 m)와 안쪽으로 말린 높은 아치 자세로 시작한다. 시뮬 `Shoot`은 기존처럼 즉시 공을 발사한다.
슈터 GUI로 발사하면 기본 경로는 월드 ground-truth 궤적에서 목표를 고른 뒤
실기와 같은 `Planner::ball_alignment`로 레일·팔을 함께 움직인다.
위치·속도 불확실성 기준을 처음 통과한 본 예측에서만 움직이고, 검증된 `y=0.08~0.35m` 구간에서 타격점을 고른다.
예측 위치 뒤 최대 10cm에서 팔을 접어 정렬하고, 타격 0.40초 전에 백스윙 없이 j0~j3 직진 푸시를 시작한다. 타격 후에는 레일을 움직이지 않고 관절만 안쪽으로 말린 자세로 복귀한다.
...
시작 시 +X(max) 엔드스톱으로 레일을 홈잉해 영점을 저장하고 0.675m 중앙으로 복귀한 뒤 기본 관절 자세를 만든다. 공 검출 시
...
기준으로 Dynamixel 관절만 최신 타격점으로 미세 보정한다. 실물이 +X로 빗나간 실측을 반영해
레일 물리 원점은 탁구대 월드 X에서 +9.025cm로 배치하고, 목표 x에는 남은 -3.475cm 실측 보정을 적용한다. 테이블·관절·토크·레일 한계를 통과한 IK 해만 실행한다.
안전한 경로가 없으면 해당 공만
...
전체 팔·레일 IK는 라켓 면을 수평 법선으로 둔 채 예측 접촉점 뒤 최대 10cm에 팔을 접어 정렬한다. 관절 한계로 10cm를 도달하지 못하면 7.5cm·5cm·3cm·2cm 순으로 가장 긴 실행 가능 거리를 선택하며, 타격 때 별도 후퇴 없이 j0~j3로 곧바로 전진한다.
...
공 궤적에서 선택한 목표 x·y·z를 임팩트 지점으로 사용한다. 안정 기준 전에는
레일·팔에 아무 명령도 보내지 않는다. 안정 기준을 처음 통과하면 리니어 목표를 한 번만
먼저 보내고, 레일 이동과 병렬로 도착 위치 기준 팔 IK를 계산한다. 이후 갱신은 레일을
건드리지 않고 팔 관절만 계속 미세 보정한다. 전역 IK 후보는 CPU 코어에 병렬 분배한다.
타격 0.40초 전에 접힌 정렬 자세에서 별도 백스윙 없이 j0~j3 직진 푸시를 시작한다. 10cm 전진 구간의 임팩트 목표 속도는 0.85m/s다. 임팩트 후 0.12초 팔로스루 뒤 리니어 레일은 현재 x에 두고 관절만 안쪽으로 말린 높은 아치 자세로 복귀한다.
정렬 중이어도 다관절 타격 명령이 관절 스트림을 선점하며, 관절 한계·토크·테이블 검사를 통과하지 못하면 타격 동작을 생략한다.
...
최종 라켓 측정 자세는 모델 1cm에서 실물 4.3cm였던 높이 오차 3.3cm를 전용 진단에만
반영한다. 홈 위치에서 수평으로 4cm 넘게 뻗는 해는 거부하고, 전완 링크의 기존 3cm
상판 안전여유는 유지한다.
