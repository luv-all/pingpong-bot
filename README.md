...
0.20초 전에 별도 백스윙 없이 j0~j3로 라켓을 바로 전진시킨다. 정렬된 라켓 면을
...
첫 유효 예측부터 즉시 움직이고, 검증된 `y=0.08~0.35m` 구간에서 타격점을 고른다.
예측 위치 뒤 최대 10cm에서 팔을 접어 정렬하고, 타격 0.20초 전에 백스윙 없이 j0~j3 직진 푸시를 시작한다. 타격 후에는 레일을 움직이지 않고 관절만 안쪽으로 말린 자세로 복귀한다.
...
기준으로 Dynamixel 관절만 최신 타격점으로 미세 보정한다. 목표 x는 별도 좌우 보정 없이
예측값을 그대로 쓰고 테이블·관절·토크·레일 한계를 통과한 IK 해만 실행한다.
안전한 경로가 없으면 해당 공만
...
공 궤적에서 선택한 목표 x·y·z를 임팩트 지점으로 사용한다. 안정 기준 전에는 현재
레일에서 팔만 예비 미세 보정한다. 안정 기준을 처음 통과하면 리니어 목표를 한 번만
먼저 보내고, 레일 이동과 병렬로 도착 위치 기준 팔 IK를 계산한다. 이후 갱신은 레일을
건드리지 않고 팔 관절만 계속 미세 보정한다. 전역 IK 후보는 CPU 코어에 병렬 분배한다.
타격 0.20초 전에 접힌 정렬 자세에서 별도 백스윙 없이 j0~j3 직진 푸시를 시작한다. 10cm 전진 구간의 임팩트 목표 속도는 0.85m/s다. 임팩트 후 0.12초 팔로스루 뒤 리니어 레일은 현재 x에 두고 관절만 안쪽으로 말린 높은 아치 자세로 복귀한다.
...
# 레일 스케일 전용 점검 — 홈잉, 중앙↔-X 50cm, 중앙↔+X 50cm, 라켓 끝 1cm 기준 자세 후 종료
...
| `--rail-scale-check` | off | 카메라 없이 중앙 기준 -X/+X 각각 50cm 스케일을 점검하고 라켓 끝을 상판 위 1cm 기준 자세로 이동한 뒤 종료 |
