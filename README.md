...
Planner::ball_alignment` 경로로 본 예측의 레일·팔 정렬 궤적을 계산한다. 이후 갱신도
레일과 전체 팔 자세를 함께 다시 풀어
보정된 공 접촉점을 라켓 블레이드에 맞추고, 라켓 면은 네트 너머 상대편
탁구대 반코트의 무게중심을 향한다. 정렬 단계부터 15° 상향각과
상대 코트 3/4 중심 방향을 맞춘다. 타격 직전 q2·q3 스냅은 사용하지
않고, 계획 FK 접촉점이 예측 공 위치에서 5 mm 이내인 정지 자세를 충돌
후까지 유지한 뒤 준비 자세로 복귀한다. 공 도착 40 ms 전까지
정렬을 끝낼 수 없는 후보는 명령하지 않는다.
리니어 레일 `+2.5 cm`는 타격 목표가 아니라 AXL board↔domain 좌표계 보정으로 한 번만 적용한다.
...
예측 위치에 레일·전체 팔로 15°·3/4 코트 중심 방향을 정지 정렬하고, 타격 0.25초 전에 q2·q3를 돌려 방향을 유지한 뒤 중립 자세로 복귀한다.
...
전체 팔·레일 IK는 라켓 면을 15° 상향각으로 상대 코트 3/4 중심을 향하게 하며 예측 공 높이보다 1.5cm 높은 지점에 정렬한다. 이 접촉점을 충돌 후까지 정지 유지한다.
...
본 예측 한 번으로 레일·전체 팔 접촉 자세를 확정한다. 종단 FK 접촉점이
예측 공 위치에서 5 mm를 넘으면 명령을 보내지 않는다. 정렬 후에는 새 예측이
충돌 직전 자세를 덮어쓰지 않게 하고, 예측 임팩트 후까지 정지 유지한 뒤
현재 모드의 준비 자세로 복귀한다.
...
`0.705 m`에 발사기 기준 오른쪽 정렬 보정 `0.025 m`를 더한 기존
`0.730 m`에, 실물 영점 보정으로 레일 전체 길이의 절반에서 +X `0.030 m`를
되돌린 `0.675 m`를 더한 제어 좌표 `1.405 m`를 쓴다. 준비 위치
`0.675 m`는 보드 `+0.730 m`에 대응하며, 기존보다 물리 -X 방향으로
`0.675 m` 이동한다.
