...
**우선순위 감각:** 시뮬 URDF·EKF(대체로 완료) → **비전 OpenCV 이전(infra)·실물 캡처** → (스핀/Magnus) → 모터 → Rerun/리플레이.
...
## 3. 관측 파이프라인 (OpenCV / infra)

비전 SSOT는 OpenCV(infra `vision`). domain DLT·`CameraSource`/`Detector`는 제거됨.

- [x] domain `triangulator` / `camera` → infra `vision` (nalgebra DLT 폴백 + `opencv` feature)
- [ ] OpenCV **원/공 검출** (`detect_*` 툴 → infra `vision`)
- [x] ChArUco 초안 (`calib_charuco --emit-sim` / `--features opencv --from-images`)
- [ ] ChArUco **완전한** `calibrateCameraCharuco` 인트린식·왜곡
- [ ] UVC / 글로벌 셔터 카메라 (`VideoCapture` 등, Windows)
- [ ] 멀티캠 동기·타임스탬프 (120 Hz 가정)
- [ ] 검출 내부 ROI(관심 창) 추적 안정화 — 별도 `Roi` 타입 없음
...
같은 infra vision API 뒤 교체 전제. 지금은 고전 파이프라인 우선.
...
cargo run -p pingpong-bin -- config/ekf-swing.toml
```

작성: 2026-07-12. 갱신: 2026-07-15 — thin types(`Prediction` 직통)·4dof URDF 머지·비전=OpenCV(infra) 정책.
