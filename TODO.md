...
- **레일·라켓 자세 공통 제어 완료:** `DirectController`를 real·GUI sim이
  공유해 목표 선택, 중앙 반환 법선, 전체 자세 IK, 안전 궤적을 같이 계산한다.
- **2단계 예측 제어 완료:** 추적 성립 즉시 1차 레일 위치로 이동하고,
  관측 0.25초·최근 3회 10cm 수렴 후 상대편 중앙 반환 자세를 명령한다.
...
- **안전 중단 추가:** 예상 도착 후 500ms까지 수렴하지 못한 상태가 3회
  연속이면 레일을 정지하고 전체 관절을 현재 위치에 홀드한 뒤 제어를 종료한다.
- **로그 정리:** 관전 창의 공 감지 상태는 `false ↔ true`로 바뀔 때만 한 번 기록한다.
- `fly_07/08/09` 검출·삼각측량·EKF 예측 진단 통과. 단, 이것은 아직
  `Target → 위치 이동`까지의 통합 테스트가 아니다.
- 현재 변경은 정적 검사와 관련 단위 테스트를 통과했다. 시뮬레이션은 실행하지
  않았고, Windows 실물 장비에서의 최종 수렴 동작은 별도 검증이 필요하다.
...
## 2. 현재 활성 제어 — 리니어 레일·중앙 반환 자세 2단계

현재 런타임은 동적 스윙을 실행하지 않는다. `Provisional`은 레일을 선행
이동하고, `Refined`는 반환 탄도에서 라켓 법선을 구한 뒤 전체 자세 IK를 푼다.
...
- [x] 상대 코트 중앙 바운드 반환 속도와 필요 라켓 법선 계산
- [x] 라켓 중심 위치·법선을 만족하는 레일·전체 관절 IK 계산
- [x] 관절·레일 속도·가속도, 토크, 테이블 충돌을 검사하는 정지→정지 궤적 생성
- [x] 공 도착까지 시간이 부족하면 하드웨어 명령 전송 전 거부
- [x] 레일 목표를 `0.000~1.410m` 물리 범위로 제한
- [x] `Provisional`은 준비 자세, `Refined`는 중앙 반환 전체 자세 사용
- [x] 달성 라켓 법선이 요구 법선에서 10° 초과하면 명령 거부
...
- [x] 예상 도착 시점부터 20ms 간격으로 레일·전체 관절 재측정
- [x] 레일 20mm·관절 3°·조준 10° 안에 2회 연속 들어오면 `converged`
- [x] 정밀 명령이 1차 명령을 덮으면 기존 측정을 `superseded`로 종료
- [x] 예상 도착 후 500ms까지 수렴하지 못하면 `timeout`
- [x] `timeout` 3회 연속이면 레일 정지·전체 관절 홀드 후 제어 종료
...
정지→정지 위치 플래너는 정밀 자세와 선택적 `--home`에 쓴다. 임팩트
속도를 만드는 동적 스윙 플래너는 real·GUI sim 직접 제어에서 아직 쓰지 않는다.
