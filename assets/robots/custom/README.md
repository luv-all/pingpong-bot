...
```bash
cp config/example.toml config/custom-robot.toml
...
- root→EE는 fixed + revolute/continuous 직렬 체인이어야 한다. prismatic·분기 제어 체인은 시작 오류
- 4축 이상이면 `plan_swing`의 racket-open 명령은 마지막 actuated 관절을 손목으로 간주
- mesh 충돌은 아직 없음
