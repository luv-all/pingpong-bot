...
use clap::Parser;
use pingpong_app::{PipelineConfig, find_robot, robot_ids_csv};
use pingpong_domain::{Arm, BallEkf, CameraId, HitPlane};
use pingpong_infra::{
...
fn main() -> Result<()> {
    ...
    let physics = runtime.physics_params();
    info!(
        path = %args.config.display(),
        mode = ?runtime.mode,
        cameras = runtime.camera_count,
        hit_plane_y = runtime.hit_plane_y,
        robot = %runtime.robot,
