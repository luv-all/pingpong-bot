...
use crate::constants::{
    geometry::{
        LINK_FOREARM_RADIUS, LINK_UPPER_RADIUS, RACKET_HALF_X, RACKET_HALF_Y, RACKET_HALF_Z,
        TABLE_CLEARANCE, TABLE_CLAMP_ITERS,
    },
    table,
};
...
/// 디버그·시각화용 — 상완 포함 전체 링크·라켓 OBB.
pub fn robot_obbs_all(arm: &Arm, rail_x: f64, joints: &Joints) -> Vec<OrientedBox> {
    let Some((base, elbow, _wrist)) = arm.chain_points(rail_x, joints) else {
        return Vec::new();
    };
    let mut boxes = Vec::with_capacity(3);
    if let Some(upper) = OrientedBox::from_segment(base, elbow, LINK_UPPER_RADIUS) {
        boxes.push(upper);
    }
    boxes.extend(robot_obbs(arm, rail_x, joints));
    return boxes;
}
