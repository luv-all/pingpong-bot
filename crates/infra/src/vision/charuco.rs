...
use opencv::objdetect::{
    CharucoBoard, CharucoDetector, CharucoParameters, DetectorParameters, Dictionary,
    PREDEFINED_DICTIONARY_NAME_DICT_4X4_50,
};
use opencv::prelude::*;
...
pub fn calibrate_charuco_draft(dir: &Path) -> Result<Calibration, String> {
    let dict = Dictionary::get(PREDEFINED_DICTIONARY_NAME_DICT_4X4_50)
        .map_err(|e| format!("dictionary: {e}"))?;
    let board = CharucoBoard::new(Size::new(5, 7), 0.04, 0.02, &dict, &Vector::new())
        .map_err(|e| format!("board: {e}"))?;
    let detector = CharucoDetector::new(
        board,
        CharucoParameters::default().ok(),
        DetectorParameters::default().ok(),
        None,
    )
