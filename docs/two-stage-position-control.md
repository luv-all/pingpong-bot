...
- 2단계 정렬: 공이 탁구대 y의 `60%` 지점을 지나면 첫 예비 명령을 보내고, 기존 불확실성 기준을 만족한 최신 예측이 오면 두 번째 정밀 명령을 보낸다.
- 타격 위치: 예측 x를 발사기 기준 오른쪽 `3cm`로 보정하고, 공은 블레이드 중심보다 `0.5cm` 아래에 닿도록 정렬한다.
- 관절: `READY_JOINTS_4DOF`
- 시작 관절 보정: 2° 기준을 완화하지 않고, 직전 모터 목표에 실측 잔여 오차를 누적해 최대 6회 폐루프 보정한다.
