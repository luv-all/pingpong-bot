...
- 레일: 실기 안전 범위 `0.0100~1.3395m`, 보드 0에 대응하는 제어 원점은 발사기 기준 오른쪽 2.5cm 정렬 보정을 포함한 `0.730m`, 준비 위치는 `RAIL_READY_X_M = 0.675m`(보드 `+0.055m`)
- 타격 위치: 공은 블레이드 중심보다 `1.5cm` 아래에 닿도록 정렬한다(기존 3cm에서 1.5cm 상향).
