...
공 x는 발사기 기준 오른쪽 3cm로 보정하고, 공이 블레이드 중심보다 0.5cm
아래에 닿도록 라켓 중심을 놓는다. 라켓 면은 네트 너머 상대편 반코트의
...
- AXL 보드 0에 대응하는 제어 원점: `1.405m` (물리 -X `0.675m` 보정 포함)
- 기본 준비 위치: `RAIL_READY_X_M = 0.675m`(보드 `+0.730m`)
