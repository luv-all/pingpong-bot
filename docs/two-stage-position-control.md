...
레일과 팔을 동시에 이동시키며 그 지점에서 백스윙 없는 짧은 타격 속도를 낸다.
...
선택 공 위치 (x,y,z)
    ...
    → 레일·팔 동시 이동 궤적 검사
    → 원래 예측 위치를 임팩트 knot로 실행
    → 라켓 면 법선 방향 짧은 팔로스루
    → 중립 자세 복귀
```

라켓 면 방향은 상대 네트 중앙을 향하도록 계산한다. 결합 궤적은 원래 예측 위치를
임팩트 knot로 삼아 그 순간 최대 1.80m/s의 전진 속도를 만든다. 백스윙과 공 도착
시각 동기화는 계산하지 않는다.
...
- 공통 위치·방향·타격 플래너: `Planner::ball_alignment_strike`
