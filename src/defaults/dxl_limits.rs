...
///
/// **0.8 → 0.9 (2026-08-05).** 발사기 실기 정렬 동작이 느린 현상을 줄이기 위해
/// 기존 상한을 12.5% 올렸다. 하드웨어 `Profile Velocity=0`은 최대 속도를
/// 허용하며, 이 값은 계획기가 만드는 관절 궤적의 속도 상한이다.
///
/// 옛 0.5는 근거가 없었다 — 바로 위
/// 속도와 토크는 물리적 성격이 다르다. 토크는 이번 짧은 위치 차단 동작에서 100%를
/// 쓰지만, 속도는 무부하 RPM을 그대로 도달 가능 속도로 보지 않고 0.9를 사용한다.
///
/// 두 논거가 갈린다:
///
...
///
/// 후자를 택해 0.9로 둔다. 실기 명령 후 관절 추종 오차가 커지면 다시 낮춰야 한다.
...
/// 다시 박을 것.
pub const JOINT_SPEED_DERATE: f64 = 0.9;
