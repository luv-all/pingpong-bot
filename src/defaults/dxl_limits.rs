...
/// [`CONTINUOUS_TORQUE_DERATE`]는 WP8 재검토와 `docs/measure-physics.md` 참조를 달고 있지만,
/// 속도 쪽은 그 값을 따라 적은 것으로 보인다. 둘은 물리적 성격이 다르다: 토크 디레이트는
/// **열·연속정격** 때문에 정당한데, 스윙은 0.3초짜리라 열이 문제되지 않는다.
///
/// 두 논거가 갈린다:
///
