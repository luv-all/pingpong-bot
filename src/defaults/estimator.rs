...
pub struct EstimatorParams {
    ...
    ///
    /// 위 표는 옛 슈터 클립(fly_02~04) 기준이라 사람 서브(fly_45~53)에 그대로
    /// 안 맞을 수 있다. 2026-08-12: 0.30(+`ALIGNMENT_TRIGGER_TABLE_Y_FRAC` 0.85)으로
    /// 느슨하게 해서 `clip-review --all`을 돌려 봤는데 RMSE가 9개 중 8개에서 더
    /// 나빠졌다(예: fly_45 11.9→58.6cm) — 관측이 적은 채로 트리거를 굳혀 조건이
    /// 나쁜 적합을 그대로 커밋해 버림. 0.15로 되돌림.
