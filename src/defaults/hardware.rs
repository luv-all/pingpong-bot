...
pub const WRIST_JOINT_ZERO_OFFSET_RAD: f64 = -12.0_f64.to_radians();
...
impl Default for DynamixelConfig {
    ...
    fn default() -> Self {
        return Self {
            ...
            //
            // **2026-08-13 벤치 실측으로 스윙 백스윙 확보.** 케이블·자기 간섭 한계를
            // 직접 재본 값은 tick 1400(forward)~3100(backward)이었다. 최댓값은 여유를 두고
            // tick 3000까지만 연다 — deg = ticks*360/4096, 즉 [123.046875, 263.671875].
            // 예전 [90,220](−40°~+90° 관절 기준)은 전방만 허용해 스윙 백스윙이 안 됐다.
            //
            // j2·j3도 같은 날 벤치 실측으로 갱신. j2는 tick 740~2750
            // (URDF 전체 65~265보다 상단이 좁다 — 실측이 URDF 가정보다 우선).
            // j3는 tick 900~3413 (URDF 전체 60~300과 거의 같음); 다만 상단 근처는
            // 라켓이 로봇 몸체에 닿아 실제 스윙에서는 쓰이지 않는다 — 모터 한계는
            // 하드웨어 최후 보호막일 뿐, 자가충돌 회피는 플래너/충돌검사 몫이다.
            motor_angle_limits_deg: vec![
                [123.046875, 263.671875],
                [135.0, 225.0],
                [65.0390625, 241.69921875],
                [79.1015625, 299.970703125],
            ],
