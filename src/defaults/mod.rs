...
pub use motion::{
    ...
    ALIGNMENT_TARGET_HEIGHT_OFFSET_M, COARSE_TRACK_JOINT_FRACTION, FIXED_JOINT_SWING_DURATION_SECS,
    HOME_RETURN_SPEED_RATIO, JACOBIAN_DAMPING, JDOT_STEP, MAGNUS_OMEGA_MAX, MAX_INTERCEPT_SAMPLES,
    MAX_PLAN_TIME_SECS, MIN_TIME_TO_GO_SECS, PLAN_DT_SECS, POSITION_TOLERANCE_RAD_OR_M,
    POST_ALIGNMENT_HOLD_SECS, RACKET_DIRECTION_TOLERANCE_DEG, RACKET_SPEED_RATIO_TOLERANCE,
...
mod tests {
    ...
    fn presets_validate() {
        ...
        assert!((rail_config.board_zero_domain_m - 1.4050).abs() < 1e-12);
        assert!((rail_config.domain_to_board_abs(0.0100) - 1.3950).abs() < 1e-12);
        assert!((rail_config.domain_to_board_abs(1.3395) - 0.0655).abs() < 1e-12);
        assert!((rail_config.domain_to_board_abs(0.6750) - 0.7300).abs() < 1e-12);
        assert!((ALIGNMENT_CONTACT_BELOW_RACKET_CENTER_M - 0.0050).abs() < 1e-12);
        assert!((ALIGNMENT_LAUNCHER_RIGHT_OFFSET_M - 0.0600).abs() < 1e-12);
        assert!((ALIGNMENT_RAIL_POSITIVE_X_CORRECTION_M - 0.0250).abs() < 1e-12);
