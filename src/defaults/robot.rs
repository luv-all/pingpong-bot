...
// 2026-08-12 실기 시작 로그의 반복 안정 도달값을 반영한다.
// 이전 `[0.05, 0.0, 1.2, -1.5721]`은 사진만 보고 모델에서 추정한
// 값이어서, 실물은 q3=-51.3°에 있는데 수렴 검사는 -90.1°를
// 요구해 매번 -38.7°로 실패했다. 아래는 로그의 q2·q3
// q0만 ID1 소프트 한계 안쪽인 0.10rad로 두고, q2·q3는
// 사진 기준의 원래 높은 아치 목표로 복구한다. ID5가 q3 목표에
// 도달하지 못한 원인은 실측값을 기본자세로 바꾸지 않고 하드웨어
// Min/Max Position Limit과 구동 상태에서 찾는다.
