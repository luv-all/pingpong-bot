...
impl RoiTrack {
    ...
    fn roi_rect(prev: camera::Pixel, half: i32, frame: &Frame) -> Option<Rect> {
    ...
    fn pick_in_frame(&mut self, frame: &Frame) -> Option<camera::Pixel> {
    ...
    fn detect_region(&mut self, frame: &Frame, roi: Option<Rect>) -> Option<camera::Pixel> {
        let Some(r) = roi else {
        ...
        return self
            .pick_in_frame(&local)
            .map(|p| camera::Pixel::new(p.x + f64::from(r.x), p.y + f64::from(r.y)));
    }
