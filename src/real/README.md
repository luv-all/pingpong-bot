# `src/real` — 라켓 헤드·리니어 레일 2단계 제어

`--mode real` 런타임은 카메라로 공을 추적하고, 라켓 헤드 x가 예측한
공 x와 같아지도록 AXL 리니어 레일을 이동한다. 동시에 Dynamixel ID 3
수평 조준축을 레일 위치의 함수로 회전해 라켓이 상대편 탁구대 끝선 중앙을 바라보게 한다.

현재 목적은 발사기 반복 시험이다. 적응형 전체 스윙 대신 짧은 고정 임팩트 푸시를 쓴다.

## 실행 범위

- 기본 `--home=true`에서 카메라·추정·제어 스레드를 시작하기 전에 레일을 실기 보정
  준비 위치(0.710 m)로, 네 관절을 준비 자세로 이동한다.
- 초기 중앙 이동이 실패하면 잘못된 위치에서 계속하지 않고 제어를 종료한다.
- 공마다 `Provisional`·`Refined` 중 먼저 성공한 한 단계만 보낸다. 그 순간 `CommandLatch`가 해당 `track_seq`를 영구히 막아 나머지 단계는 보내지 않는다.
- 레일은 두 단계 모두 라켓 헤드 x가 선택 목표 x와 맞도록 움직인다.
- 조준축은 두 단계 모두 `atan2(상대편 끝선 중앙 x - rail_x, 끝선 y - 마운트 y)`를 쓴다.
- 나머지 Dynamixel 축에는 추적 중 Goal Position을 다시 보내지 않는다.
- 레일·조준 명령과 거의 동시에 라켓 법선 방향 5cm 고정 푸시를 시작한다.
- 임팩트 목표 라켓 속도는 최대 출력 시험용 1.80m/s이고, 관절은 실기 모델 속도
  상한까지 사용한다. 0.06초 팔로스루 뒤 중앙 준비 자세로 복귀한다.

## 데이터 흐름

```mermaid
flowchart LR
    camL["왼쪽 카메라"] --> est["삼각측량 · EKF"]
    camR["오른쪽 카메라"] --> est
    est --> stage["목표 선택 · 1/2차 분류"]
    stage -->|"CommitRequest 최신 1개"| shared["DirectController 공통 명령 계산"]
    shared --> ctl["control_worker"]
    ctl --> rail["AXL 리니어 레일"]
    ctl --> aim["Dynamixel ID 3 수평 조준"]
    ctl --> impact["4축 고정 임팩트 푸시"]
    rail --> readback["명령 후 재측정"]
    aim --> readback
    impact --> home["중앙 준비 자세 복귀"]
    est --> preview["프리뷰 · 목표 표시"]
```

카메라, 추정기, 하드웨어는 각 워커가 단독 소유한다. 채널이 가득 차면
오래된 프레임이나 요청을 버리고 최신 값만 남긴다.

## 제어 단계

`PredictionStability`가 선택 목표의 안정도를 분류한다.

- `Provisional`: EKF가 위치와 속도를 추정하고 목표를 선택할 수 있는 즉시
- `Refined`: 첫 정상 관측 후 0.25초 이상이며 최근 목표 3개가 최신 목표에서 10cm 이내

각 `CommitRequest`에는 `track_seq`가 들어간다. 추정기가 새 공을 발견하면 번호를
올리고, 제어 워커는 번호가 바뀌는 즉시 단계별 중복 방지 상태를 초기화한다.
따라서 이전 공이 `Refined`까지 가지 못했어도 다음 공의 `Provisional` 명령이 막히지 않는다.

## 제어 워커

`control_worker`는 다음 순서로 요청을 처리한다.

1. 50ms보다 오래된 요청을 버린다.
2. 직전 명령 후 20ms가 지나지 않았으면 버린다.
3. `y=0.08~0.35m` 구간에서 목표 하나를 선택한다.
4. 레일 위치→조준각 함수와 FK로 헤드 x 오프셋을 보정한 레일 목표를 구한다.
5. 현재 위치와 레일 가속·속도, 조준축 속도로 시간 내 도달 가능한지 검사한다.
6. 레일 목표와 조준 목표를 함께 하드웨어에 보내고 실제 적용값을 받는다.
7. 예상 도착 시점부터 20ms 간격으로 재측정해 허용치 안에 2회 연속 들어오면 완료한다.
8. 예상 도착 후 500ms 타임아웃이 3회 연속이면 장치를 정지하고 제어를 종료한다.
9. 레일·조준과 관절 푸시를 함께 시작하고, 남은 비행시간에 맞춰 공 도착 시 임팩트한다.
10. 짧은 팔로스루가 끝나면 `return_to_center`로 레일·관절을 복귀시킨다.

위 3~5번은 `robot::control::DirectController`에 있으며 GUI 시뮬레이션도
같은 계산을 사용한다. 실기는 명령 후 엔코더를 읽고, 시뮬레이션은
같은 명령을 `robot::State`의 레일·조준 목표에 적용한다.

> **주의 (2026-08-05):** 위 7~8번(재측정 수렴 판정·3회 연속 실패 시 중단)은
> 현재 실기 `spawn()` 루프에서 발동하지 않는다 — `pending_verification`이
> 유닛 테스트 밖에서는 채워지지 않는다. 상세:
> `docs/superpowers/specs/2026-08-05-control-worker-state-machine-design.md`.

### 제어 괴리 로그

- 레일: commanded, measured, commanded-minus-measured를 m 단위 소수점 4자리로 기록
- AXL 시작 시 ActPos와 CmdPos 원점 차이를 기록하고 실제 위치 기준으로 명령을 보정
- 조준축: 같은 세 값을 rad·deg 둘 다 기록
- 로그 필드의 commanded는 하드웨어가 반환한 applied이며, 제어기의 requested는 별도 기록
- 정밀 명령으로 덮어쓴 1차 명령: `superseded`로 당시 실측값 보존
- 레일 오차 20mm 또는 조준축 오차 3° 초과: `WARN`
- 허용치 이내: `INFO`

### 관전 창 공 상태 로그

관전용 자식 프로세스의 `ball=false`/`ball=true` 로그는 매 패킷마다 나오지 않는다.
첫 추정 패킷의 상태를 한 번 기록한 뒤 실제 감지 상태가 `false ↔ true`로 바뀔
때만 다시 기록한다. 제어 워커가 보내는 포즈 전용 패킷의 `ball=None`은 공이
사라졌다는 뜻이 아니라 해당 필드를 보내지 않았다는 뜻이므로 상태를 바꾸지 않는다.

## 하드웨어 경계

`Hardware::command_rail_and_racket`이 단순 제어 전용 명령이다.

- AXL은 매 단계의 최신 목표를 받으며, 가속·감속 램프를 포함해 명령 시간에 맞는 속도를 계산한다.
- 정렬 중에는 `DynamixelBus::write_joint(1, ...)`으로 논리 관절 1만 쓴다.
- 기본 모터 배열 `[1, 3, 4, 5]`에서 논리 관절 1은 실제 ID 3이다.
- 모터 각도와 레일 위치는 기존 하드웨어 한계로 제한되며 적용된 값이 호출자에 반환된다.

전체 궤적용 `Hardware::command`는 시작 홈 이동과 중앙 복귀에 사용한다.
고정 푸시는 `Hardware::command_joints`로 네 관절만 구동하므로, 동시에 진행 중인
AXL 레일 직접 이동을 중단하거나 목표를 덮어쓰지 않는다.

## 런타임 이벤트

메인 스레드에는 실제로 발생하는 상태만 보낸다.

- `Ready`: 하드웨어 초기화와 선택적 홈 이동 완료
- `Tracking`: 새 공의 위치·속도 추정 시작
- `Commanded`: 레일·조준 명령 전송 완료(하드웨어가 반환한 실제 적용값 포함)
- `Failed`: 하드웨어 오류
- `Done`: 제어 워커 종료

구형 스윙 상태인 `Committed`, `Infeasible`, `PlanFailed`, `Recovering`은 사용하지 않는다.

## 프리뷰와 관전 창

- 초록 원: 현재 프레임의 2D 검출
- 흰 원: 생 삼각측량 점 재투영
- 빨간 원: 현재 선택한 제어 목표
- HUD: 추적 여부, `Provisional/Refined`, 목표 좌표와 남은 시간

관전 창 메시지는 공 위치, 선택 목표, 시작 포즈만 전달한다. 실기 임팩트 궤적 자체를
관전 창에 전달하거나 재생하지는 않는다.

## 주요 파일

| 파일 | 역할 |
|---|---|
| `run.rs` | 워커·채널 조립, 이벤트 로그, 종료 |
| `camera_worker.rs` | 캡처와 검출 |
| `estimator_worker.rs` | 삼각측량, EKF, 목표 선택, 단계 분류 |
| `commit_request.rs` | `track_seq + BallTrajectory + PredictionStage` |
| `control_worker.rs` | 단계별 중복 방지, 레일·조준, 임팩트 시각과 자동 복귀 |
| `runtime_event.rs` | 현재 런타임 이벤트 |
| `preview.rs` | 검출·목표 HUD |
| `sim_update.rs` | 관전 창용 공·목표·포즈 메시지 |

## 주의

실물 제어는 Windows와 AXL DLL이 필요하다. 종료 시 기본적으로 Dynamixel 토크를
유지하므로 손으로 축을 움직이려면 `--release-torque`를 사용하거나 전원을 내려야 한다.
