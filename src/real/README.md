...
현재 목적은 발사기 반복 시험이다. 시작부터 팔을 살짝 감아 두고, 검출 후 추가
백스윙과 공 높이 정렬을 거쳐 상대편을 향해 펴지는 임팩트를 쓴다.

## 실행 범위

- 기본 `--home=true`에서 카메라·추정·제어 스레드를 시작하기 전에 라켓을 중앙
  `y=0.215m`, `z=1.05m` 부근에서 상대편을 향한 채 2cm 감긴 준비 자세로 이동한다.
- 초기 중앙 이동이 실패하면 잘못된 위치에서 계속하지 않고 제어를 종료한다.
- 공마다 `Provisional`·`Refined` 중 먼저 성공한 한 단계만 보낸다. 그 순간 `CommandLatch`가 해당 `track_seq`를 영구히 막아 나머지 단계는 보내지 않는다.
- 레일은 두 단계 모두 라켓 헤드 x가 선택 목표 x와 맞도록 움직인다.
- 조준축은 두 단계 모두 `atan2(상대편 끝선 중앙 x - rail_x, 끝선 y - 마운트 y)`를 쓴다.
- 공 검출 시 예측 공 위치 `(x,y,z)`와 상대편 끝선 중앙 방향을 함께 만족하는
  4축 임팩트 자세를 구한다. 현재 준비 자세에 가까운 IK 해를 골라 팔이 다른 방향으로
  다시 말리는 것을 막는다.
- 레일 이동과 함께 2cm 감김을 유지한 채 예측 공 높이로 정렬한다. 기본 발사 높이에서는
  검출 후 뒤로 더 감지 않는다.
- 기본 접촉점은 라켓 중심이 공 중심보다 2cm 아래이며, 라켓 면은 상대편 방향에서
  8° 위를 향한다. 충돌·관절 한계가 있으면 1cm 아래→정중앙→1cm 위 순서로 재계산한다.
- 임팩트 목표 라켓 속도는 최대 출력 시험용 1.80m/s이고, 관절은 실기 모델 속도
  상한까지 사용한다. 전진 임팩트 시간은 백스윙 가능 여부 판정에서 제외하고,
  백스윙 뒤 실제 남은 시간을 그대로 쓴다. 공 도착 시 원래 조준 자세로 펴지며,
  0.06초 팔로스루 뒤 감긴 중앙 준비 자세로 복귀한다.
...
flowchart LR
    ...
    ctl --> windup["2cm 감김 유지 · 높이 정렬"]
    windup --> impact["상대편 방향 임팩트"]
    rail --> readback["명령 후 재측정"]
    aim --> readback
    impact --> home["중앙 준비 자세 복귀"]
...
9. 현재 준비 자세에 가까운 임팩트 IK를 구해 공 높이를 맞추고, 레일 이동과 추가
   백스윙을 함께 시작한다.
10. 백스윙이 끝나면 남은 비행시간에 맞춰 팔을 펴 공 도착 시 상대편 끝선 중앙
    방향 자세로 임팩트한다.
11. 짧은 팔로스루가 끝나면 `ready_prewind`로 감긴 중앙 준비 자세에 복귀한다.
12. 특정 공의 궤적이 시간·관절·테이블 한계를 만족하지 못하면 그 공만 건너뛰고,
    같은 `track_seq`의 재시도를 막은 뒤 다음 공을 계속 기다린다.
13. 직접 중앙 복귀가 테이블을 스치면, 실행 전 검증된 상승 중간 자세를 거쳐 복귀한다.
    안전한 전체 경로가 없으면 모터에 보내지 않고 현재 자세를 유지한다.
...
> **주의 (2026-08-05):** 위 7~8번(재측정 수렴 판정·3회 연속 실패 시 중단)은
...
- 정렬 중에는 `DynamixelBus::write_joint(1, ...)`으로 논리 관절 1만 쓴다.
- 기본 모터 배열 `[1, 3, 4, 5]`에서 논리 관절 1은 실제 ID 3이다.
- 모터 각도와 레일 위치는 기존 하드웨어 한계로 제한되며 적용된 값이 호출자에 반환된다.

전체 궤적용 `Hardware::command`는 사용하지 않고, 시작 준비·백스윙·임팩트·복귀의
관절 구간은 `Hardware::command_joints`로 구동한다. 따라서 동시에 진행 중인 AXL
레일 직접 이동을 중단하거나 목표를 덮어쓰지 않는다.
...
관전 창 메시지는 공 위치, 선택 목표, 시작 포즈만 전달한다. 실기 임팩트 궤적 자체를
관전 창에 전달하거나 재생하지는 않는다.
...
| `control_worker.rs` | 단계별 중복 방지, 레일·조준, 임팩트 시각과 자동 복귀 |
