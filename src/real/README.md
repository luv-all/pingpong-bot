...
- 레일·조준이 끝나면 공 도착 0.12초 전에 라켓 법선 방향 2.5cm 고정 푸시를 시작한다.
- 임팩트 목표 라켓 속도는 0.65m/s이고 0.06초 팔로스루 뒤 중앙 준비 자세로 복귀한다.
...
9. 공 도착 0.12초 전에 고정 임팩트 푸시를 실행한다.
...
전체 궤적용 `Hardware::command`는 시작 홈 이동과 고정 임팩트 푸시에 사용한다.
고정 푸시에서는 레일을 고정하고 네 관절을 같은 짧은 궤적으로 구동한다.
