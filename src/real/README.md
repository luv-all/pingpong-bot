# `src/real` — 라켓 헤드·리니어 레일 2단계 제어

`--mode real` 런타임은 카메라로 공을 추적하고, 라켓 헤드 x가 예측한
공 x와 같아지도록 AXL 리니어 레일을 이동한다. 동시에 Dynamixel ID 3
수평 조준축을 레일 위치의 함수로 회전해 라켓이 상대편 탁구대 끝선 중앙을 바라보게 한다.

현재 목적은 장비 응답 확인이다. 공을 되치는 전체 스윙 제어가 아니다.

## 실행 범위

- 기본 `--home=true`에서 레일을 실기 보정 준비 위치(0.760 m)로, 네 관절을 준비 자세로 이동한다.
- 초기 중앙 이동이 실패하면 잘못된 위치에서 계속하지 않고 제어를 종료한다.
- 공마다 `Provisional`과 `Refined` 명령을 최대 한 번씩 보낸다.
- 레일은 두 단계 모두 라켓 헤드 x가 선택 목표 x와 맞도록 움직인다.
- 조준축은 두 단계 모두 `atan2(상대편 끝선 중앙 x - rail_x, 끝선 y - 마운트 y)`를 쓴다.
- 나머지 Dynamixel 축에는 추적 중 Goal Position을 다시 보내지 않는다.
- IK, 전체 자세 계획, 타격 속도, 팔로스루는 실행하지 않는다.
- 공의 최신 선택 목표 시각이 지나고 명령 수렴 확인이 끝나면 중앙 준비 자세로 복귀한다.
...
4. 레일 위치→조준각 함수와 FK로 헤드 x 오프셋을 보정한 레일 목표를 구한다.
5. 현재 위치와 레일 가속·속도, 조준축 속도로 시간 내 도달 가능한지 검사한다.
6. 레일 목표와 조준 목표를 함께 하드웨어에 보내고 실제 적용값을 받는다.
7. 예상 도착 시점부터 20ms 간격으로 재측정해 허용치 안에 2회 연속 들어오면 완료한다.
8. 예상 도착 후 500ms 타임아웃이 3회 연속이면 장치를 정지하고 제어를 종료한다.
9. 최신 공 목표 시각이 지나면 기존 `return_to_center` 궤적으로 레일·관절을 복귀시킨다.
...
- 레일 오차 20mm 또는 조준축 오차 3° 초과: `WARN`
- 허용치 이내: `INFO`
...
- `DynamixelBus::write_joint(1, ...)`으로 논리 관절 1만 쓴다.
- 기본 모터 배열 `[1, 3, 4, 5]`에서 논리 관절 1은 실제 ID 3이다.
...
- `Commanded`: 레일·조준 명령 전송 완료(하드웨어가 반환한 실제 적용값 포함)
- `Failed`: 하드웨어 오류
- `Done`: 제어 워커 종료
...
| `control_worker.rs` | 단계별 중복 방지와 레일·조준 명령 |
