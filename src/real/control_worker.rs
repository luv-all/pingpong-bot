...
//! 0.50초 전에 q3 손목만 움직여 타격 순간 라켓 면을 25°로 만든다. 손목 동작이 불가하면 그 동작만 생략하고
...
const FIXED_SWING_LEAD: Duration = Duration::from_millis(500);
...
mod tests {
    ...
    impl Hardware for SharedPoseHardware {
        ...
        fn command_rail(&mut self, rail_x: f64, _duration_secs: f64) -> Result<f64, HwError> {
            self.pose.lock().expect("lock").rail_x = rail_x;
            return Ok(rail_x);
        }
    }
