...
pub fn spawn(
    ...
    return thread::spawn(move || {
        ...
        info!(
            ...
            "공 위치·방향 정렬 준비 — 본 예측 레일 1회, 후속 팔 보정, 다관절 푸시"
        );
        ...
        // 시간이 되면 진행 중 관절 궤적을 선점해 본 예측의 후속 미세 보정을 이어가며,
                ...
                match hardware.read_pose() {
                    Ok(pose) => {
                        ...
                        info!(
                            ...
                            "타격 후 Dynamixel만 준비 자세 복귀 — 레일 타격 위치 유지, 바로 다음 공 대기"
                        );
                    }
                    Err(error) => warn!(%error, "준비 자세 복귀 후 포즈 읽기 실패"),
                }
                // 실시간 랠리를 위해 타격 후 바로 Idle로 전환해 다음 공을 즉시 받는다.
...
fn move_to_ready(hardware: &mut dyn Hardware, arm: &Arm, rail_x: f64) -> Result<(), MoveError> {
    ...
    let trajectories = plan_neutral_return_segments(
        &ready_arm,
        &start,
        rail_x,
        pingpong_bot::defaults::HOME_RETURN_SPEED_RATIO,
...
/// 타격 후 레일은 현재 위치에 고정하고 Dynamixel 관절만 안쪽으로 말린 자세로 복귀한다.
...
fn move_joints_to_ready_in_place(hardware: &mut dyn Hardware, arm: &Arm) -> Result<(), MoveError> {
    ...
    let trajectories = plan_neutral_return_segments(
        ...
        &start,
        start.rail_x,
        pingpong_bot::defaults::RALLY_RETURN_SPEED_RATIO,
    )
    .map_err(MoveError::Plan)?;
    for trajectory in trajectories {
        hardware
...
fn plan_neutral_return_segments(
    ...
    for lift_m in [0.03, 0.06, 0.10, 0.15] {
        ...
        match Planner::return_to_center_at_speed_ratio(arm, &lifted_pose, rail_x, speed_ratio) {
...
mod tests {
    ...
    fn startup_initialization_keeps_safe_partially_tucked_pose_and_centers_rail() {
        ...
        assert!(
            hardware
                .rail_targets
                .iter()
                .all(|target| (*target - rail.x_max).abs() > 1e-12),
            "자세 초기화가 예전 +X 끝 왕복을 실행하면 안 됨"
        );
    }
    ...
    fn logged_follow_through_pose_has_a_safe_ready_return() {
        ...
        let segments = plan_neutral_return_segments(
            &robot.arm,
            &start,
            rail.default_x(),
            pingpong_bot::defaults::HOME_RETURN_SPEED_RATIO,
    ...
    fn plan_neutral_return_segments_is_slower_than_full_speed_return() {
        ...
        let home_segments = plan_neutral_return_segments(
            &robot.arm,
            &start,
            rail.x_min,
            pingpong_bot::defaults::HOME_RETURN_SPEED_RATIO,
    ...
    #[test]
    fn spawn_auto_returns_to_idle_immediately_after_post_hit_return() {
        let robot = pingpong_bot::defaults::robot().expect("robot");
        let rail = robot.arm.rail.expect("레일 있는 로봇");
        ...
        commit_tx
            ...
            .expect("공 예측 전송");
        // 실시간 랠리: 타격 후 Waiting을 거치지 않고 바로 Idle로 돌아와 다음
        // 공을 즉시 받을 수 있어야 한다.
