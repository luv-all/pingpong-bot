...
fn ball_alignment_geometry_with_prewind(
    ball: Point3,
    ...
    // 위치 IK에서 정한 면을 타격 중에도 유지해 라켓 중심이 뒤로 휘지 않게 한다.
    let target_normal = horizontal_normal;
    let contact_offset = crate::constants::BALL_RADIUS
...
pub fn ball_alignment_pose(
    ...
    for prewind_distance_m in [FIXED_JOINT_PUSH_DISTANCE_M, 0.025, 0.020] {
        ...
        match result {
            Ok(aligned_pose) => return Ok(aligned_pose),
            Err(error) => last_error = Some(error),
        }
    }
    return Err(last_error.expect("정렬 준비 거리 후보가 하나 이상 있어야 함"));
}
...
/// 팔 관절을 움직이지 않고 현재 라켓 중심 x를 공 x에 맞추는 레일 목표.
///
/// 고정 스윙 폴백은 자세 IK를 쓰지 않으므로, 현재 FK의 라켓 x 오프셋만큼
/// 레일을 평행이동해야 실제 라켓 중심이 공 x에 놓인다.
pub fn ball_x_tracking_rail_target(arm: &Arm, start: &robot::Pose, ball_x: f64) -> f64 {
    let target = arm
        ...
        .map_or(ball_x, |racket| start.rail_x + ball_x - racket.position.x);
    return arm
        .rail
        .as_ref()
        .map_or(target, |rail| rail.clamp_x(target));
}
    ...
    for prewind_distance_m in [FIXED_JOINT_PUSH_DISTANCE_M, 0.025, 0.020] {
        ...
        let result = arm
            ...
            .and_then(|(aligned_pose, _)| {
                ...
                validate_ball_alignment_normal(corrected_ball, target_normal, reached.normal)?;
                plan_move_to(arm, start, aligned_pose.joints, start.rail_x)
            });
        match result {
...
/// 정렬 자세에서 네 관절로 곧바로 전진 푸시한 결과.
pub struct FixedJointSwing {
    pub trajectory: Trajectory,
    /// 기구학적으로 움직임이 필요하지 않아 고정된 관절.
...
fn plan_fixed_joint_swing_to_pose(
    arm: &Arm,
    start: &robot::Pose,
    ...
    let peak = impact_velocity
        ...
        .fold(0.0_f64, f64::max);
    let speed_limit = arm.max_joint_speed * FIXED_JOINT_SNAP_SPEED_RATIO;
    if peak > speed_limit && speed_limit > 0.0 {
        let scale = speed_limit / peak;
        for velocity in &mut impact_velocity {
            *velocity *= scale;
        }
    }
    let skipped_joint_indices = (0..joint_count)
        .filter(|index| {
            (impact_pose.joints.values[*index] - start.joints.values[*index]).abs() < 1e-6
                && impact_velocity[*index].abs() < 1e-6
        })
        .collect();
    let trajectory = build_feasible_trajectory_with_follow_time(
        arm,
        &start.joints,
        impact_pose.joints,
        vec![0.0; joint_count],
...
pub fn plan_fixed_joint_swing_quadratic_from_alignment(
    ...
    let try_push_distance = |push_distance_m: f64| -> Result<FixedJointSwing, DomainError> {
        ...
        return plan_fixed_joint_swing_quadratic_to_pose(
            arm,
            start,
            target_position,
            target_normal,
...
pub fn plan_fixed_joint_push_fallback(
    ...
    let joint_count = start.joints.values.len();
    let Some(wrist_index) = arm.wrist_joint_index() else {
        return Err(DomainError::InfeasibleSwing(
            SwingPlanError::JointOrTorqueLimit {
                target_x: start.rail_x,
                target_y: 0.0,
    ...
    // 보존한다: j0 +18.74°, j1 유지, j2 -24.27°. 자세 IK 실패 시에도
    // 같은 밀치기 방향을 재현하려는 폴백 전용 값이다.
    const FIXED_PUSH_DELTAS_RAD: [(usize, f64); 2] =
        [(0, 0.327_073_600_041_434_3), (2, -0.423_583_300_198_340_6)];
    for &(index, delta) in &FIXED_PUSH_DELTAS_RAD {
        let requested = impact_values[index] + delta;
        impact_values[index] = arm
            .joint_limit(index)
    ...
    let mut follow_values = impact.values.clone();
    for index in 0..joint_count {
        follow_values[index] += impact_velocity[index] * follow_time * 0.5;
        if let Some(limit) = arm.joint_limit(index) {
            follow_values[index] = follow_values[index].clamp(limit.min, limit.max);
        }
    }
    ...
    let skipped_joint_indices = (0..joint_count)
        .filter(|index| {
            (trajectory.end.values[*index] - trajectory.start.values[*index]).abs() < 1e-6
        })
        .collect();
    return Ok(FixedJointSwing {
        trajectory,
    ...
    while duration <= RETURN_TO_CENTER_MAX_SECS {
        ...
        match build_feasible_trajectory(
            ...
            Err(error @ SwingPlanError::TablePenetration { .. }) => {
                ...
                // 올린 자세)을 넣어 즉시 재시도한다.
                if let Some(lifted) = plan_move_to_via_table_clearance(
                    arm,
                    start,
                    center_joints.clone(),
                    duration,
...
fn kinematic_limit_violation(arm: &Arm, trajectory: &Trajectory) -> Option<&'static str> {
    // 프로파일 경계에서 부동소수점 반올림으로 최고속과 수 e-15만큼
    // 차이 나는 값은 동일한 명령이다. 실제 초과만 거부한다.
    if trajectory.peak_joint_speed() > arm.max_joint_speed * (1.0 + 1e-9) {
        return Some("관절 속도");
    }
...
mod tests {
    ...
    #[test]
    fn fixed_joint_swing_pushes_forward_without_backswing() {
        ...
        let aligned = arm
            .forward_kinematics_with_rail(trajectory.rail.start, &trajectory.start)
            .expect("alignment FK");
        let impact = arm
        ...
        let push_direction = Vector3::new(aligned.normal.x, aligned.normal.y, 0.0).normalize();
        let mut minimum_forward = 0.0_f64;
        for step in 0..=40 {
            let time = trajectory.impact_time_secs * f64::from(step) / 40.0;
            let sampled = arm
                .forward_kinematics_with_rail(
                    trajectory.sample_rail_at(time),
                    &trajectory.sample_at(time),
                )
                .expect("swing path FK");
            let forward = (sampled.position - aligned.position).dot(&push_direction);
            minimum_forward = minimum_forward.min(forward);
        }
        assert!(
            minimum_forward > -0.001,
            "의도적인 백스윙이 없어야 함: 최대 후퇴={:.3}mm, q0={:?}, q1={:?}",
            minimum_forward * 1e3,
            trajectory.start.values,
            trajectory.end.values,
        );
        let pushed_distance = (impact.position - aligned.position).dot(&push_direction);
        assert!(
        ...
        for (angle, velocity) in after_impact.values.iter_mut().zip(&trajectory.end_velocity) {
            *angle += velocity * velocity_probe_dt;
        }
        ...
        let impact_tilt_deg = impact
            ...
            .to_degrees();
        let aligned_tilt_deg = aligned
            .normal
            .z
            .atan2(aligned.normal.x.hypot(aligned.normal.y))
            .to_degrees();
        assert!(
            (impact_tilt_deg - aligned_tilt_deg).abs() <= 3.0,
            "백스윙을 없앤 직진 푸시는 정렬된 라켓 면을 유지해야 함: {impact_tilt_deg:.3}°"
        );
        assert_ne!(
    ...
    fn fixed_joint_swing_power_sweep_j0_j2_sustain_ceiling_speed_before_impact() {
        ...
        let planned =
            plan_fixed_joint_swing_power_sweep(arm, &home, FIXED_JOINT_SWING_MIN_IMPACT_TIME_SECS)
        ...
        for &index in &[0usize, 2usize] {
            ...
            let cruise_probe =
                trajectory.impact_time_secs - FIXED_JOINT_SWING_MIN_IMPACT_TIME_SECS * 0.5;
    ...
    fn fixed_joint_swing_power_sweep_drives_wrist_up_at_max_speed() {
        ...
        let planned =
            plan_fixed_joint_swing_power_sweep(arm, &start, FIXED_JOINT_SWING_MIN_IMPACT_TIME_SECS)
        .expect("power sweep swing");
        let trajectory = planned.trajectory;
    ...
    #[test]
    fn fixed_push_fallback_keeps_rail_fixed_and_moves_j0_j2_j3() {
        let active = crate::defaults::robot().expect("active robot");
        let arm = &active.arm;
        let ready = robot::Pose::new(
            arm.rail.as_ref().map_or(0.0, |rail| rail.default_x()),
        ...
        let planned =
            plan_fixed_joint_push_fallback(arm, &ready, FIXED_JOINT_SWING_MIN_IMPACT_TIME_SECS)
                .expect("fixed push fallback");
        let trajectory = planned.trajectory;
        assert_eq!(trajectory.rail.start, ready.rail_x);
        assert_eq!(trajectory.rail.end, ready.rail_x);
    ...
    #[test]
    fn ball_x_tracking_rail_target_aligns_current_racket_x_without_ik() {
        let active = crate::defaults::robot().expect("active robot");
        let arm = &active.arm;
        let start = robot::Pose::new(
            arm.rail.as_ref().map_or(0.0, |rail| rail.default_x()),
        ...
        let ball_x = table::WIDTH_X * 0.5;
        let rail_x = ball_x_tracking_rail_target(arm, &start, ball_x);
        let racket = arm
            .forward_kinematics_with_rail(rail_x, &start.joints)
            .expect("translated racket FK");
        assert!(
            (racket.position.x - ball_x).abs() < 1e-9,
            "racket_x={} ball_x={ball_x}",
            racket.position.x
        );
    }
    ...
    #[test]
    fn fixed_joint_swing_from_wrist_limit_stays_inside_limits() {
        let active = crate::defaults::robot().expect("active robot");
        let arm = &active.arm;
        let rail_x = arm.rail.as_ref().map_or(0.0, |rail| rail.default_x());
        let mut joints = arm.default_joints.clone();
        joints.values[3] = arm.joint_limit(3).expect("q3 limit").min;
        let start = robot::Pose::new(rail_x, joints);
        let planned = plan_fixed_joint_swing(arm, &start).expect("wrist-limit straight push");
        assert!(arm.joints_in_limits(&planned.trajectory.end));
    }
