...
use crate::eval::{LaunchParams, Mode, Progress};
...
impl PanelUiState {
    pub fn from_controls(controls: &SimRuntimeControls) -> Self {
        return Self {
            ...
            eval: Arc::new(Mutex::new(Progress::default())),
            eval_running: Arc::new(AtomicBool::new(false)),
            eval_mode: Mode::Block,
            eval_launch: LaunchParams::default(),
            eval_live: None,
        };
    }
}
