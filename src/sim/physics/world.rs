...
impl SimWorld {
    ...
    fn try_auto_swing(&mut self, _dt: f64) {
        ...
        // 테스트·디버그 경로가 직접 시작한 궤적은 자동 직접 제어가 덮어쓰지 않는다.
        // 이 제어기가 시작한 정렬은 latch에 트랙이 기록되므로 후속 보정을 계속 허용한다.
        if self.robot.is_swinging() && self.direct_alignment_latch.track_seq().is_none() {
            return;
        }
...
mod tests {
    ...
    #[test]
    #[ignore = "main 설정을 유지하면 이 고속 측면 샷은 정밀 예측 전에 지나가므로 별도 모션 튜닝이 필요함"]
    ...
    #[test]
    #[ignore = "main의 현재 rail_frame/READY 자세는 별도 shot_tune이 필요함"]
