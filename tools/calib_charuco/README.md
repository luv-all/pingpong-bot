...
# -o 생략 시 data/calibration.json (DEFAULT_CALIBRATION_PATH SSOT)
```
...
```bash
# 이미 선별한 폴더만 보정
cargo run -p calib-charuco -- --from-images ./boards/cam0 -o cam0.json

cargo run -p calib-charuco -- --emit-sim 3 -o data/calibration.json
cargo run -p calib-charuco -- --validate data/calibration.json
```
